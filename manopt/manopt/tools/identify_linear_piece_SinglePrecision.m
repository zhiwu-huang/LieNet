function [range, poly] = identify_linear_piece_SinglePrecision(x, y, window_length)
...
% Change log: 
    ...
    residues = single(zeros(length(x)-window_length, 1));
    polys = single(zeros(2, length(residues)));
