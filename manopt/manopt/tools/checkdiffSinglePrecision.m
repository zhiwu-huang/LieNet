function checkdiffSinglePrecision(problem, x, d)
...
% Change log: 
    ...
    % range given by h.
    h = single(logspace(-8, 0, 51));
    value = single(zeros(size(h)));
    for i = 1 : length(h)
        y = problem.M.exp(x, d, h(i));
        y.encoder = single(y.encoder);
        y.decoder = single(y.decoder);
    ...
    window_len = 10;
    [range poly] = identify_linear_piece_SinglePrecision(log10(h), log10(err), window_len);
    hold on;
