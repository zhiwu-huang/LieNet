function checkgradientSinglePrecision(problem, x, d)
...
% Change log: 
    ...
    x.encoder = single(x.encoder);
    x.decoder = single(x.decoder);
    d.encoder = single(d.encoder);
    d.decoder = single(d.decoder);
    ...
    checkdiffSinglePrecision(problem, x, d);
