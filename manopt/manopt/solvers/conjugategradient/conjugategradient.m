...
% % % Vijay - swap to quick cost computation using a subset of the minibatch to deal with large datasets
%     fh = problem.cost;
%     problem.cost = problem.quickcost;
%     % Execute line search
%     [stepsize newx storedb lsmem lsstats] = options.linesearch(...
%                  problem, x, desc_dir, cost, df0, options, storedb, lsmem);
%     problem.cost = fh;% swap back to original cost on full minibatch
